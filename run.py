...
def main():
    parser = argparse.ArgumentParser(
        description='Time Series Data Cleaner - 时间序列数据清理和筛选工具',
...
示例用法:
  ...
  python run.py --input data.npy --mode filter --keep_temp_files --dataset_name my_dataset
    ...
    # 零值检测参数
    parser.add_argument('--enable_zero_check', action='store_true', default=False, 
                       help='是否启用零值检测（默认：False，需显式启用）')
    parser.add_argument('--zero_check_len', type=int, default=100, 
                       help='零值检测的检查长度（默认：100）')
    ...
    parser.add_argument('--near_zero_threshold', type=float, default=0.005, 
                       help='接近零的阈值（默认：0.005）')
    parser.add_argument('--zero_std_threshold', type=float, default=1.0, 
                       help='零值检测的标准差阈值（默认：1.0）')
    
    # 处理模式
    parser.add_argument('--mode', type=str, choices=['full', 'features', 'filter'], 
                       default='full',
